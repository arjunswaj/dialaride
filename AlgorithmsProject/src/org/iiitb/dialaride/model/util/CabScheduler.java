...
public class CabScheduler {
  ...
  public void schedule(DialARideModel model, int thresholdValue) {
      ...
      for (RideRequest rideRequest : rideReqs) {
        if (model.getCost()[rideRequest.getSource()][rideRequest
          ...
          eventQueue.add(new Event(EventTypes.CUSTOMER_REQUEST, null,
              rideRequest, rideRequest.getTimeStart(), -1, -1, 0));
        }
      }
    }
    while (!eventQueue.isEmpty()) {
      ...
      case CUSTOMER_REQUEST:
        ...
        Cab found = findACab(model, event.getRideRequest(), eventQueue,
            (int) event.getTimeInstant());
        if (null != found) {
          successfulScheduling += 1;
          revenue += model.getCost()[event.getRideRequest().getSource()][event
              .getRideRequest().getDestination()];
          found.setRevenue(found.getRevenue()
              + model.getCost()[event.getRideRequest().getSource()][event
                  .getRideRequest().getDestination()]);
  ...
  private void handleReachedTargetNode(DialARideModel model, Event event,
    ...
    Node node = model.getNodes().get(event.getNodeNumber());
    IntervalTree<Interval> cabsSet = node.getCabsSet();
    cabsSet.softDelete(new Interval((int) Math.ceil(event.getTimeInstant()),
        (int) Math.ceil(event.getTimeInstant()), cab));

    cab.getPath().remove(0);
    // cab.setPassengers(cab.getPassengers() - event.getDropCounts());
    if (!isPartial) {
      cab.setDroppingAnyone(false);
      cabsSet.add(new Interval((int) Math.ceil(event.getTimeInstant()), 2880,
          cab));
      cab.setPassengers(0);
    } else {
      cab.setPassengers(cab.getPassengers() - event.getDropCounts());
  ...
  private Cab findACab(DialARideModel model, RideRequest rideRequest,
    ...
    while (!nodeQueue.isEmpty()) {
      ...
      for (Interval interval : intervals) {
        ...
        if (now < interval.getStart()) {
          ...
          int distFromSrcToNode = costMatrix[source][node.getNodeNumber()];
        ...
        if (feasible) {
          if (!cab.isPickingAnyone()
            ...
            if (cab.isDroppingAnyone()) {
              ...
              // + cab.getCabNo());
              found = checkIfPathIsSubPathAndScheduleARide(cab, rideRequest,
                  model, interval, cabsSet, timeInstant, node, eventQueue);
              if (found) {
                cab.setPassengers(cab.getPassengers() + 1);
              }
            } else {
              if (node.getNodeNumber() != source) {
                timeInstant = scheduleAPickUp(cab, model, rideRequest,
                    eventQueue, interval, cabsSet, node.getNodeNumber(),
                    source, timeInstant);
              }
              scheduleARide(cab, model, rideRequest, eventQueue, timeInstant,
                  cabsSet, interval);
              cab.setPassengers(cab.getPassengers() + 1);
            }
          }
      ...
      if (!found) {
        SortedMap<Integer, List<Neighbour>> adjNodes = node.getAdjacentNodes();
  ...
  private boolean checkIfPathIsSubPathAndScheduleARide(Cab cab,
    ...
    if (isSubPath) {
      ...
      doThePickupWhileServingSomeoneAndServeAll(model, model.getNodes(),
          rideRequest, eventQueue, timeInstant, rideRequest.getSource(), cab,
      ...
      if (isSubPath) {
        ...
        // printPath("MyPath: ", myPath);
        // printPathNode("ThatCabsPath: ", thatCabsPath);
        doThePickupWhileServingSomeoneAndServeAllWhenMyStopIsLastStop(model,
            model.getNodes(), rideRequest, eventQueue, timeInstant,
            rideRequest.getSource(), cab, node.getNodeNumber(), cabFoundInSrc,
            cabsSet, interval);
      } else {
        // System.out
  ...
  private void doThePickupWhileServingSomeoneAndServeAllWhenMyStopIsLastStop(
    ...
    if (!cabFoundInSrc) {
      ...
      pickUpNode.getCabsSet().add(
          new Interval((int) Math.ceil(actualArrivalTimeOfCab), (int) Math
              .ceil(actualArrivalTimeOfCab), scheduledCab));
      Event event = new Event(EventTypes.REACHED_PICKUP_NODE, scheduledCab,
          rideRequest, Math.ceil(actualArrivalTimeOfCab), pickUpDestination,
          scheduledCab.getVersion(), 0);
      model
          .getCabPath()
          .get(scheduledCab.getCabNo())
          .add(
              new Path(pickUpDestination, (int) Math
                  .ceil(actualArrivalTimeOfCab), EventTypes.REACHED_PICKUP_NODE));
    ...
    for (IntermediateNode intermed : intermediateNodes) {
      ...
      if (intermed.getDropCount() > 0) {
        scheduleTime = scheduleARideWithOtherPassengers(scheduledCab, model,
            pathSource, intermed.getNodeNumber(), eventQueue, scheduleTime,
            intermed.getDropCount(), false, rideRequest);
        pathSource = intermed.getNodeNumber();
      }
    }

    scheduleARideWithOtherPassengers(scheduledCab, model, pathSource,
        rideRequest.getDestination(), eventQueue, scheduleTime, 1, true,
        rideRequest);

  }

  private void doThePickupWhileServingSomeoneAndServeAll(DialARideModel model,
      List<Node> nodes, RideRequest rideRequest, Queue<Event> eventQueue,
      double currentTime, int pickUpDestination, Cab scheduledCab,
      int sourceOfVehicle, boolean cabFoundInSrc,
      IntervalTree<Interval> cabsSet, Interval interval) {
    ...
    for (IntermediateNode intermed : intermediateNodes) {
      ...
      if (intermed.getDropCount() > 0) {
        ...
        scheduleTime = scheduleARideWithOtherPassengers(scheduledCab, model,
            pathSource, intermed.getNodeNumber(), eventQueue, scheduleTime,
            intermed.getDropCount(), isLast, rideRequest);
        pathSource = intermed.getNodeNumber();
      }
      count += 1;
    }
    ...
    while (!revPath.isEmpty()) {
      ...
      if (intermediateNodeNo == pathSize) {
        ...
        model
            ...
            .get(scheduledCab.getCabNo())
            .add(
                new Path(dest, (int) Math.ceil(timeInstant),
                    EventTypes.REACHED_TARGET_NODE));
      } else {
        event = new Event(EventTypes.REACHED_NODE, scheduledCab, rideRequest,
            Math.ceil(timeInstant), dest, scheduledCab.getVersion(), 0);
  ...
  private double scheduleARideWithOtherPassengers(Cab scheduledCab,
      ...
      Queue<Event> eventQueue, double timeStart, int dropCount, boolean isLast,
    ...
    while (!revPath.isEmpty()) {
      ...
      if (intermediateNodeNo == pathSize) {
        if (isLast) {
          event = new Event(EventTypes.REACHED_TARGET_NODE, scheduledCab,
              rideRequest, Math.ceil(timeInstant), dest,
              scheduledCab.getVersion(), dropCount);
          ...
          event = new Event(EventTypes.REACHED_PARTIAL_TARGET_NODE,
              scheduledCab, rideRequest, Math.ceil(timeInstant), dest,
              scheduledCab.getVersion(), dropCount);
          for (int ctr = 0; ctr < dropCount; ctr += 1) {
            model
                .getCabPath()
        ...
        scheduledCab.getPath().add(new IntermediateNode(dest, dropCount, val));
      } else {
  ...
  private double scheduleAPickUp(Cab scheduledCab, DialARideModel model,
      RideRequest rideRequest, Queue<Event> eventQueue, Interval interval,
      IntervalTree<Interval> cabsSet, int sourceOfVehicle,
      int pickUpDestination, double currentTime) {
    List<Node> nodes = model.getNodes();
    Stack<Integer> revPath = new Stack<Integer>();
    int[][] prev = model.getPrev();
    ...
    Node pickUpNode = model.getNodes().get(pickUpDestination);
    pickUpNode.getCabsSet().add(
        new Interval((int) Math.ceil(timeInstant),
            (int) Math.ceil(timeInstant), scheduledCab));
    Event event = new Event(EventTypes.REACHED_PICKUP_NODE, scheduledCab,
