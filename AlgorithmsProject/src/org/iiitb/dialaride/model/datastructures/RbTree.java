...
public class RbTree {
	...
	/**
	 * 
	 * Test code: make sure that the tree has all the properties defined by Red
	 * Black trees:
	 * 
	 * o. Root is black.
	 * 
	 ...
	 * 
	 * o. Every path from root to leaves contains the same number of black
	 * nodes.
	 * 
	 * Calling this function will be expensive, as is meant for assertion or
	 * test code.
	 */
	public boolean isValid() {
		if (this.root.color != RbNode.BLACK) {
			// System.out.println("root color is wrong");
			return false;
		}
		if (NIL.color != RbNode.BLACK) {
			// System.out.println("NIL color is wrong");
			return false;
		}
		if (allRedNodesFollowConstraints(this.root) == false) {
			// System.out.println("red node doesn't follow constraints");
			return false;
		}
		if (isBalancedBlackHeight(this.root) == false) {
			// System.out.println("black height unbalanced");
			return false;
		}
		return true;
	}
